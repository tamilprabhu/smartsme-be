...
const createProductSchema = Joi.object({
    ...
    wastage: Joi.number().required().messages({
        'number.base': 'wastage must be a number',
        'any.required': 'wastage is required',
    }),
    norms: Joi.number().required().messages({
    ...
    salesCode: Joi.string().trim().min(1).required().messages({
    ...
    salesPercent: Joi.number().required().messages({
...
const updateProductSchema = Joi.object({
    ...
    wastage: Joi.number().optional().messages({
        'number.base': 'wastage must be a number',
    }),
    norms: Joi.number().optional().messages({
        'number.base': 'norms must be a number',
    }),
    totalWeight: Joi.number().optional().messages({
        'number.base': 'totalWeight must be a number',
    }),
    cavity: Joi.number().optional().messages({
        'number.base': 'cavity must be a number',
    }),
    shotRate: Joi.number().optional().messages({
        'number.base': 'shotRate must be a number',
    }),
    perItemRate: Joi.number().optional().messages({
        'number.base': 'perItemRate must be a number',
    }),
    incentiveLimit: Joi.number().optional().messages({
        'number.base': 'incentiveLimit must be a number',
    }),
    salesType: Joi.string().trim().min(1).optional().messages({
        'string.base': 'salesType must be a string',
        'string.empty': 'salesType cannot be blank',
        'string.min': 'salesType cannot be blank',
    }),
    salesCode: Joi.string().trim().min(1).optional().messages({
        'string.base': 'salesCode must be a string',
        'string.empty': 'salesCode cannot be blank',
        'string.min': 'salesCode cannot be blank',
    }),
    salesPercent: Joi.number().optional().messages({
        'number.base': 'salesPercent must be a number',
    }),
})
    .min(1)
...
const validateCreate = async (payload, companyId) => {
    ...
    if (!(await isProductNameUniqueWithinCompany(value.productName, companyId))) {
        throw {
            ...
            errors: { productName: ['productName must be unique within company'] },
        };
    }
...
const validateUpdate = async (id, payload, companyId) => {
    ...
    if (
        value.productName &&
        !(await isProductNameUniqueWithinCompany(value.productName, companyId, id))
    ) {
        throw {
            name: 'ValidationError',
            errors: { productName: ['productName must be unique within company'] },
