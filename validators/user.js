...
const userSchema = Joi.object({
    ...
    password: Joi.string()
        ...
        .messages({
            ...
            'string.pattern.base':
                'password must contain at least one uppercase, one lowercase, one number and one symbol, no spaces allowed',
...
const validateUpdate = async (id, userData, currentPassword) => {
    ...
    if (value.password && value.password === currentPassword) {
        throw {
            ...
            errors: { password: ['password must be different from current password'] },
        };
    }
