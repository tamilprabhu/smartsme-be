...
const companySchema = Joi.object({
    companyName: Joi.string()
        ...
        .messages({
            ...
            'string.pattern.base':
                'companyName must be alphanumeric only, no special characters except spaces, hyphens, commas, and dots',
            'any.required': 'companyName is required',
        }),

    companyType: Joi.string().valid('LARGE', 'MEDIUM', 'SMALL').required().messages({
        'any.only': 'companyType must be one of LARGE, MEDIUM, or SMALL',
        'any.required': 'companyType is required',
    }),

    address: Joi.string().trim().min(10).max(50).required().messages({
    ...
    propName: Joi.string()
        ...
        .messages({
            ...
            'string.pattern.base':
                'propName must be alphanumeric only, no special characters except spaces, hyphens, commas, and dots',
    ...
    mailId: Joi.string().trim().email().required().messages({
    ...
    natureOfBusiness: Joi.string()
        ...
        .messages({
            ...
            'string.pattern.base':
                'natureOfBusiness must be alphanumeric only, no special characters except spaces',
    ...
    authPerson: Joi.string()
        ...
        .messages({
            ...
            'string.pattern.base':
                'authPerson must be alphanumeric only, no special characters except spaces, hyphens, commas, and dots',
