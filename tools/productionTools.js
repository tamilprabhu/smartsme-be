...
const productionReportTool = tool(
    async ({ companyId, productId, startDate, endDate }) => {
        try {
            ...
            const totalProduction = filteredData.reduce(
                (sum, shift) => sum + (shift.production || 0),
                0,
            );
            const totalRejection = filteredData.reduce(
                (sum, shift) => sum + (shift.rejection || 0),
                0,
            );
            const totalNetProduction = filteredData.reduce(
                (sum, shift) => sum + (shift.netProduction || 0),
                0,
            );
            ...
            return JSON.stringify({
                ...
                summary: {
                    ...
                    rejectionRate:
                        totalProduction > 0
                            ? ((totalRejection / totalProduction) * 100).toFixed(2)
                            : 0,
                },
    ...
    },
    {
        ...
        description:
            'Generate production report for a specific product or all products within a date range',
        schema: z.object({
            ...
            startDate: z
                .string()
                .optional()
                .describe('Start date in YYYY-MM-DD format (optional, defaults to 7 days ago)'),
            endDate: z
                .string()
                .optional()
