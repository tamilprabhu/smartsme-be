...
const machineService = {
    ...
    getAllMachines: async (
        ...
        logger.info(
            `MachineService: Fetching machines - page: ${page}, itemsPerPage: ${validLimit}, search: ${search}, companyId: ${companyId}`,
        );
        try {
            ...
            logger.info(
                `MachineService: Successfully retrieved ${rows.length} machines out of ${count} total for company: ${companyId}`,
            );
            return {
                items: rows,
                paging: {
    ...
    getMachineById: async (id, companyId = null) => {
        ...
        try {
            ...
            if (machine) {
                logger.info(
                    `MachineService: Successfully retrieved machine: ${machine.machineName} (ID: ${id})`,
                    {
                ...
                logger.warn(
                    `MachineService: Machine not found with ID: ${id} for company: ${companyId}`,
                );
            }
            return machine;
        } catch (error) {
    ...
    createMachine: async (machineData, companyId, userId) => {
        ...
        try {
            ...
            while (attempts < MAX_RETRY_ATTEMPTS) {
                try {
                    ...
                    const isUniqueError =
                        ...
                        error.errors?.some(
                            (e) => e.path === 'machine_id' || e.path === 'machineId',
                        );
                    ...
                    if (isUniqueError) {
                        ...
                        if (attempts >= MAX_RETRY_ATTEMPTS) {
                            throw new Error(
                                'Failed to generate unique machine_id after maximum retries',
                            );
                        }
                        logger.warn(
                            `MachineService: Duplicate machine_id, retrying (${attempts}/${MAX_RETRY_ATTEMPTS})`,
                        );
                    } else {
                        throw error;
                    }
            ...
            logger.info(
                `MachineService: Successfully created machine: ${machine.machineName} (ID: ${machine.machineSequence})`,
                {
    ...
    updateMachine: async (id, machineData, companyId, userId) => {
        logger.info(`MachineService: Updating machine with ID: ${id}`, {
        ...
        try {
            ...
            if (updatedRows === 0) {
                logger.warn(
                    `MachineService: No machine found to update with ID: ${id} for company: ${companyId}`,
                );
                throw new Error('Machine not found');
            }
            const updatedMachine = await Machine.findOne({ where: whereClause });
            logger.info(
                `MachineService: Successfully updated machine: ${updatedMachine.machineName} (ID: ${id})`,
            );
    ...
    deleteMachine: async (id, companyId) => {
        ...
        try {
            ...
            if (updatedRows === 0) {
                logger.warn(
                    `MachineService: No machine found to delete with ID: ${id} for company: ${companyId}`,
                );
                throw new Error('Machine not found');
            }
            logger.info(
                `MachineService: Successfully soft deleted machine: ${machine?.machineName || 'Unknown'} (ID: ${id})`,
            );
