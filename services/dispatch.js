...
const dispatchService = {
    getAllDispatches: async (
        ...
        logger.info(
            `DispatchService: Fetching dispatches - page: ${page}, itemsPerPage: ${validLimit}, search: ${search}, companyId: ${companyId}`,
        );
        try {
            ...
            logger.info(
                `DispatchService: Successfully retrieved ${rows.length} dispatches out of ${count} total for company: ${companyId}`,
            );
            return {
                items: rows,
                paging: {
    ...
    getDispatchById: async (id, companyId = null) => {
        ...
        try {
            ...
            if (dispatch) {
                logger.info(
                    `DispatchService: Successfully retrieved dispatch: ${dispatch.dispatchId} (ID: ${id}) for company: ${companyId}`,
                );
            } else {
                logger.warn(
                    `DispatchService: Dispatch not found with ID: ${id} for company: ${companyId}`,
                );
            }
            return dispatch;
        } catch (error) {
    ...
    createDispatch: async (dispatchData, companyId = null, userId = null) => {
        ...
        try {
            ...
            while (attempts < MAX_RETRY_ATTEMPTS) {
                try {
                    ...
                    const isUniqueError =
                        ...
                        error.errors?.some(
                            (e) => e.path === 'dispatch_id' || e.path === 'dispatchId',
                        );
                    ...
                    if (isUniqueError) {
                        ...
                        if (attempts >= MAX_RETRY_ATTEMPTS) {
                            throw new Error(
                                'Failed to generate unique dispatch_id after maximum retries',
                            );
                        }
                        logger.warn(
                            `DispatchService: Duplicate dispatch_id, retrying (${attempts}/${MAX_RETRY_ATTEMPTS})`,
                        );
                    } else {
                        throw error;
                    }
            ...
            logger.info(
                `DispatchService: Successfully created dispatch: ${dispatch.dispatchId} (ID: ${dispatch.dispatchSequence}) for company: ${companyId}`,
            );
            return dispatch;
        } catch (error) {
    ...
    updateDispatch: async (id, dispatchData, companyId = null, userId = null) => {
        ...
        try {
            ...
            if (updatedRows === 0) {
                logger.warn(
                    `DispatchService: No dispatch found to update with ID: ${id} for company: ${companyId}`,
                );
                throw new Error('Dispatch not found');
            }
            const updatedDispatch = await Dispatch.findOne({ where: whereClause });
            logger.info(
                `DispatchService: Successfully updated dispatch: ${updatedDispatch.dispatchId} (ID: ${id}) for company: ${companyId}`,
            );
    ...
    deleteDispatch: async (id, companyId = null, userId = null) => {
        ...
        try {
            ...
            if (updatedRows === 0) {
                logger.warn(
                    `DispatchService: No dispatch found to delete with ID: ${id} for company: ${companyId}`,
                );
                throw new Error('Dispatch not found');
            }
            logger.info(
                `DispatchService: Successfully soft deleted dispatch: ${dispatch?.dispatchId || 'Unknown'} (ID: ${id}) for company: ${companyId} by user: ${userId}`,
            );
