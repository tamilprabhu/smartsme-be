...
const orderService = {
    ...
    getAllOrders: async (
        ...
        logger.info(
            `OrderService: Fetching orders - page: ${page}, itemsPerPage: ${validLimit}, search: ${search}, companyId: ${companyId}`,
        );
        try {
            ...
            logger.info(
                `OrderService: Successfully retrieved ${rows.length} orders out of ${count} total for company ${companyId}`,
            );
            return {
                items: rows,
                paging: {
    ...
    getOrderById: async (id, companyId) => {
        ...
        try {
            ...
            if (order) {
                logger.info(
                    `OrderService: Successfully retrieved order: ${order.orderId} (ID: ${id}) for company: ${companyId}`,
                    {
                ...
                logger.warn(
                    `OrderService: Order not found with ID: ${id} for company: ${companyId}`,
                );
            }
            return order;
        } catch (error) {
            logger.error(
                `OrderService: Failed to fetch order with ID: ${id} for company: ${companyId}`,
                {
                    error: error.message,
                    stack: error.stack,
                },
    ...
    createOrder: async (orderData, companyId, userId) => {
        logger.info(
            `OrderService: Creating new order for company: ${companyId} by user: ${userId}`,
            {
        ...
        try {
                ...
                try {
                    ...
                    if (isUniqueError) {
                        ...
                        if (attempts >= MAX_RETRY_ATTEMPTS) {
                            throw new Error(
                                'Failed to generate unique order_id after maximum retries',
                            );
                        }
                        logger.warn(
                            `OrderService: Duplicate order_id, retrying (${attempts}/${MAX_RETRY_ATTEMPTS})`,
                        );
                    } else {
                        throw error;
                    }
            ...
            logger.info(
                `OrderService: Successfully created order: ${order.orderId} (ID: ${order.orderSequence}) for company: ${companyId}`,
                {
            ...
            logger.error(
                `OrderService: Failed to create order: ${orderData.orderId} for company: ${companyId}`,
                {
                    error: error.message,
                    orderData: orderData,
                    stack: error.stack,
    ...
    updateOrder: async (id, orderData, companyId, userId) => {
        logger.info(`OrderService: Updating order with ID: ${id}`, {
        ...
        try {
            ...
            logger.info(
                `OrderService: Successfully updated order: ${updatedOrder.orderId} (ID: ${id})`,
                {
    ...
    deleteOrder: async (id, companyId) => {
        ...
        try {
            ...
            logger.info(
                `OrderService: Successfully soft deleted order: ${order?.orderId || 'Unknown'} (ID: ${id})`,
            );
