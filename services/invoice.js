...
const invoiceService = {
    ...
    getAllInvoices: async (
        ...
        logger.info(
            `InvoiceService: Fetching invoices - page: ${page}, itemsPerPage: ${validLimit}, search: ${search}, companyId: ${companyId}`,
        );
        try {
            ...
            logger.info(
                `InvoiceService: Successfully retrieved ${rows.length} invoices out of ${count} total for company: ${companyId}`,
            );
            return {
                items: rows,
                paging: {
    ...
    getInvoiceById: async (id, companyId = null) => {
        ...
        try {
            ...
            if (invoice) {
                logger.info(
                    `InvoiceService: Successfully retrieved invoice: ${invoice.invoiceId} (ID: ${id}) for company: ${companyId}`,
                );
            } else {
                logger.warn(
                    `InvoiceService: Invoice not found with ID: ${id} for company: ${companyId}`,
                );
            }
            return invoice;
        } catch (error) {
    ...
    createInvoice: async (invoiceData, companyId = null, userId = null) => {
        ...
        try {
            ...
            while (attempts < MAX_RETRY_ATTEMPTS) {
                try {
                    ...
                    const isUniqueError =
                        ...
                        error.errors?.some(
                            (e) => e.path === 'invoice_id' || e.path === 'invoiceId',
                        );
                    ...
                    if (isUniqueError) {
                        ...
                        if (attempts >= MAX_RETRY_ATTEMPTS) {
                            throw new Error(
                                'Failed to generate unique invoice_id after maximum retries',
                            );
                        }
                        logger.warn(
                            `InvoiceService: Duplicate invoice_id, retrying (${attempts}/${MAX_RETRY_ATTEMPTS})`,
                        );
                    } else {
                        throw error;
                    }
            ...
            logger.info(
                `InvoiceService: Successfully created invoice: ${invoice.invoiceId} (ID: ${invoice.invoiceSequence}) for company: ${companyId}`,
            );
            return invoice;
        } catch (error) {
    ...
    updateInvoice: async (id, invoiceData, companyId = null, userId = null) => {
        ...
        try {
            ...
            if (updatedRows === 0) {
                logger.warn(
                    `InvoiceService: No invoice found to update with ID: ${id} for company: ${companyId}`,
                );
                throw new Error('Invoice not found');
            }
            const updatedInvoice = await Invoice.findOne({ where: whereClause });
            logger.info(
                `InvoiceService: Successfully updated invoice: ${updatedInvoice.invoiceId} (ID: ${id}) for company: ${companyId}`,
            );
    ...
    deleteInvoice: async (id, companyId = null, userId = null) => {
        ...
        try {
            ...
            if (updatedRows === 0) {
                logger.warn(
                    `InvoiceService: No invoice found to delete with ID: ${id} for company: ${companyId}`,
                );
                throw new Error('Invoice not found');
            }
            logger.info(
                `InvoiceService: Successfully soft deleted invoice: ${invoice?.invoiceId || 'Unknown'} (ID: ${id}) for company: ${companyId}`,
            );
