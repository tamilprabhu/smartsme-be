...
const MAX_UPLOAD_BYTES = Number.isFinite(parseInt(process.env.UPLOAD_MAX_BYTES || '', 10))
    ...
    : Number.isFinite(MAX_UPLOAD_MB)
      ? MAX_UPLOAD_MB * 1024 * 1024
      : 100 * 1024 * 1024;
const STORAGE_ROOT =
    process.env.ASSET_STORAGE_ROOT || path.join(__dirname, '..', 'storage', 'uploads');
...
const safeSegment = (value) =>
    normalizeSegment(value).replace(/[^a-zA-Z0-9._-]/g, '_') || 'unknown';
...
const handleUpload = async (req, res) => {
    ...
    try {
        ...
        if (!moduleName || !subModuleName || !identifier) {
            return res
                .status(400)
                .json({ error: 'module, subModule, and identifier are required' });
        }
...
router.get('/', optionalAuth, async (req, res) => {
    ...
    try {
        ...
        const itemsPerPage = ItemsPerPage.isValid(req.query.itemsPerPage)
    ...
    try {
        ...
        if (asset.visibility === 'private') {
            ...
            if (
                ...
                decoded.assetId?.toString() !== asset.id.toString()
            ) {
