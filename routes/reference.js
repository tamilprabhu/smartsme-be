...
router.get('/states', authenticate, async (req, res) => {
    ...
    try {
        ...
        logger.info(`GET /reference/states - Successfully retrieved ${states.length} states`, {
            requestId,
            username,
        });
        res.json(states);
...
router.get('/districts/:stateId', authenticate, async (req, res) => {
    ...
    logger.info(`GET /reference/districts/${stateId} - Fetching districts`, {
        requestId,
        username,
        stateId,
    });
    ...
    try {
        ...
        logger.info(
            `GET /reference/districts/${stateId} - Successfully retrieved ${districts.length} districts`,
            {
                requestId,
                username,
                stateId,
...
router.get('/pincodes', authenticate, async (req, res) => {
    ...
    logger.info(`GET /reference/pincodes - Searching pincode: ${pincode}`, {
        requestId,
        username,
        pincode,
    });
